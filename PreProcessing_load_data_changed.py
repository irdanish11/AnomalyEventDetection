...
#@njit    
def PreProcessing(img_name, read_path, canny_edge=True, canny_path=None, sigma=0.33):
    """
...
def load_data( canned_frames, seq_size = 8):
  '''
  ...
  for i in tqdm(range(len(cany_imgs)//seq_size)):
    seq = []
...
if __name__=='__main__':    
    ...
    for i in tqdm(range(len(onlyfiles))):
        ...
        for img in images:
            ...
            write_path = 'test_ProcessedImages/'+dirs[i]
            canny_path = 'test_CannyImages/'+dirs[i]
            rescaled_image = PreProcessing(img_name, read_path=img, canny_edge=True,
